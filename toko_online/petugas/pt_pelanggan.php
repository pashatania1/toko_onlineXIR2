...
if(isset($_POST["submit"])) {
    ...
    if(empty($nama)){
        ...
        echo "<script>alert('password pelanggan tidak boleh kosong');location.href='t_pelanggan.php';</script>"; 
        ...
        echo "<script>alert('foto pelanggan tidak boleh kosong');location.href='t_pelanggan.php';</script>"; 
    } else {
        ...
        // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                ...
                $p = md5($password);
                $sql = "INSERT INTO `pelanggan`(`nama`, `username`, `password`, `alamat`, `telp`, `foto`) VALUES ('$nama', '$username', '$p', '$alamat', '$telp', '$foto')";
                
                $insert=mysqli_query($conn, $sql);

                if($insert) {
                    echo "<script>alert('Sukses menambahkan pelanggan');location.href='tam_pelanggan.php';</script>";
                } else {
                    echo "<script>alert('Gagal menambahkan pelanggan');location.href='t_pelanggan.php';</script>";
                }
            } else {
